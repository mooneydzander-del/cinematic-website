...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const payload = {
    ...
    business_name:     String(body.business_name     || body.business || body.businessName || '').trim(),
    website_url:       String(body.website_url       || body.website  || '').trim(),
    industry:          String(body.industry          || '').trim(),
    offer:             String(body.offer             || '').trim(),
    traffic_source:    String(body.traffic_source    || body.trafficSource || '').trim(),
    running_ads:       String(body.running_ads       || body.runningAds || body.goal || '').trim(),
    landing_page_goal: String(body.landing_page_goal || body.goal     || '').trim(),
    budget:            String(body.budget            || '').trim(),
    timeline:          String(body.timeline          || '').trim(),
    message:           String(body.message           || body.notes    || '').trim(),
    source_page:       String(body.source_page       || body.source   || '').trim(),
    submission_time:   String(body.submission_time   || body.submittedAt || new Date().toISOString()).trim(),
  };
