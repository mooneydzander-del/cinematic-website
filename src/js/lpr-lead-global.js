document.addEventListener("DOMContentLoaded", function () {
  console.log("LPR global lead handler loaded");

  async function submitLPRLead(form, event) {
    ...
    console.log("LPR lead form submitted", form);
    ...
    try {
      const response = await fetch("/api/lpr-lead", {
        method: "POST",
        headers: {
      ...
      console.log("LPR lead response status", response.status);

      const result = await response.json();
      console.log("LPR lead response data", result);
      ...
      console.error("LPR lead submit failed", error);
  ...
  document.addEventListener("submit", function (event) {
    ...
    if (
      ...
      form.tagName === "FORM" &&
      (
        form.id === "lpr-lead-form" ||
        form.classList.contains("lpr-lead-form") ||
        form.querySelector("#lpr-lead-submit") ||
        form.querySelector(".lpr-lead-submit")
      )
    ) {
      submitLPRLead(form, event);
    }
  }, true);

  document.addEventListener("click", function (event) {
    const button = event.target.closest("#lpr-lead-submit, .lpr-lead-submit");
    ...
    if (!form) {
      console.error("LPR lead submit button clicked but no parent form found");
      return;
    }
